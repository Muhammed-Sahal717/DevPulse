...
async def fetch_data_repository_stats(
    project_id: int,
):  # Keeping function name identical
        ...
        if repo_response.status_code == 200:
            ...
            with Session(bg_engine) as session:
                ...
                if db_project:
                    ...
                    session.commit()
                    print(
                        f"[BACKGROUND WORKER] ✅ Real metrics successfully synced for Project {project_id}!\n"
                    )
        else:
            print(
                f"[BACKGROUND WORKER] ❌ GitHub API responded with status: {repo_response.status_code}"
            )
